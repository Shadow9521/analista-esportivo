    ...
    if etapa < len(fatores):
        ...
        with col_grafico:
            ...
            data_comparativo = pd.DataFrame({
                "Resultado": [f"{time_casa} 🏠", "Empate 🤝", f"{time_fora} 🛫"] + [f"{time_casa} 🏠", "Empate 🤝", f"{time_fora} 🛫"],
...
# === PARTE 3: Resultado Final após o checklist ===
if st.session_state.fase == 3:
    st.subheader("📌 Resultado da Análise Final")
    ...
    with col1:
        st.metric("Prob. Vitória", f"{vitoria}%", delta=f"Odd Justa: {odds_justas['Vitória']}")
    with col2:
        st.metric("Prob. Empate", f"{empate}%", delta=f"Odd Justa: {odds_justas['Empate']}")
    with col3:
        st.metric("Prob. Derrota", f"{derrota}%", delta=f"Odd Justa: {odds_justas['Derrota']}")

    st.markdown("### 💸 Recomendacão de Aposta (Fórmula de Kelly)")

    melhores_mercados = []
    for nome, prob, odd_real in zip(["Vitória", "Empate", "Derrota"], [vitoria, empate, derrota], [odd_vitoria, odd_empate, odd_derrota]):
        prob_decimal = prob / 100
        kelly = kelly_formula(prob_decimal, odd_real - 1)
        valor_aposta = round(banca * kelly, 2)
        if valor_aposta > 0:
            melhores_mercados.append((nome, valor_aposta, round(kelly * 100, 2)))

    if melhores_mercados:
        for nome, valor_aposta, percentual in melhores_mercados:
            st.success(f"✅ Apostar em **{nome}** com R$ {valor_aposta} ({percentual}%)")
    else:
        st.warning("Nenhum mercado é recomendado com base na Fórmula de Kelly.")

    # Respostas do checklist
    st.markdown("### 🔍 Detalhamento do Checklist")
    for _, msg in respostas:
        st.markdown(f"- {msg}")

    # Exportar para Excel
    st.markdown("### 📁 Exportar Análise")
    df_export = pd.DataFrame([{ "Critério": msg, "Peso": peso } for peso, msg in respostas])
    df_export["Jogo"] = f"{time_casa} x {time_fora}"
    df_export["Banca"] = banca
    df_export["Recomendação"] = ', '.join([nome for nome, _, _ in melhores_mercados]) or "Nenhuma"
    df_export["Probabilidades"] = f"{vitoria}/{empate}/{derrota}"

    excel_data = export_df_to_excel(df_export)
    st.download_button(label="📥 Baixar Análise em Excel", data=excel_data, file_name="analise_aposta.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
