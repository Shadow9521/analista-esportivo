...
# Perguntas reformuladas para evitar ambiguidade
fatores = [
...
for i, (pergunta, peso) in enumerate(fatores):
    with st.expander(f"{i+1}. {pergunta}"):
        ...
        if escolha == time_casa:
            ...
            contribuicoes.append(f"✅ {pergunta} → {time_casa} ({'+' if peso > 0 else ''}{peso}%)")
        elif escolha == time_fora:
            probabilidade_base -= peso
            contribuicoes.append(f"❌ {pergunta} → {time_fora} ({'-' if peso > 0 else '+'}{abs(peso)}%)")
        else:
