...
jogos_proximos = pd.DataFrame({
    "Data": ["2025-06-12", "2025-06-13", "2025-06-14", "2025-06-15"],
    "Jogo": ["Brasil x Uruguai", "França x Itália", "Alemanha x Espanha", "Argentina x Colômbia"],
    "Campeonato": ["Eliminatórias", "Eurocopa", "Eurocopa", "Copa América"]
})

filtro_data = st.sidebar.date_input("Filtrar por data", value=None)
filtro_campeonato = st.sidebar.selectbox("Filtrar por campeonato", ["Todos"] + sorted(jogos_proximos["Campeonato"].unique().tolist()))

df_filtrado = jogos_proximos.copy()
if filtro_campeonato != "Todos":
    df_filtrado = df_filtrado[df_filtrado["Campeonato"] == filtro_campeonato]
if filtro_data:
    df_filtrado = df_filtrado[df_filtrado["Data"] == filtro_data.strftime("%Y-%m-%d")]

st.sidebar.write("### Jogos Filtrados")
st.sidebar.dataframe(df_filtrado, use_container_width=True)

jogo_selecionado = st.sidebar.selectbox("Selecione o Jogo para Analisar", df_filtrado["Jogo"] if not df_filtrado.empty else [""])
...
if jogo_selecionado:
    ...
    analise["comentarios"] = st.text_area("Comentários, observações ou insights sobre este jogo:", value=analise["comentarios"], height=150)
else:
    st.info("Nenhum jogo disponível com os filtros selecionados.")
