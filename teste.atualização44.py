...
if etapa < len(fatores):
    # Cálculo parcial e gráfico dinâmico durante o checklist
    ...
    fig_bar.update_layout(yaxis_range=[0, 100], height=350, margin=dict(t=50, b=50))
    ...
    for _, mensagem in respostas:
        st.markdown(f"- {mensagem}")

    saldo_time_casa = sum([peso for peso, mensagem in respostas if '⬆️' in mensagem])
    saldo_time_fora = sum([-peso for peso, mensagem in respostas if '⬇️' in mensagem])
