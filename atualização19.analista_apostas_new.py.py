...
def comparar_valores(val_a, val_b, time_casa, time_fora):
    ...
    vb = converter(val_b)
    if va > vb:
        return +2, f"⬆️ Elenco mais valioso → {time_casa} ({val_a} vs {val_b})"
    elif vb > va:
        return -2, f"⬇️ Elenco mais valioso → {time_fora} ({val_b} vs {val_a})"
    else:
        return 0, f"⚖️ Elencos de valor similar ({val_a} = {val_b})"
...
if modo == "Checklist Automático":
    ...
    # URLs dos times no Transfermarkt (ajustar conforme o jogo)
    ...
    peso_valor, texto_valor = comparar_valores(valor_a, valor_b, time_casa, time_fora)
