...
if etapa < len(fatores):
    ...
    with col_grafico:
        fig = px.bar(data_grafico, x="Resultado", y="Probabilidade", text="Odd", color="Resultado",
                     color_discrete_sequence=["#00cc96", "#636efa", "#ef553b"])
        fig.update_traces(texttemplate="Odd: %{text:.2f}", textposition="outside")
        fig.update_layout(yaxis_range=[0, 100], height=320, margin=dict(t=30, b=20), title="")
        st.plotly_chart(fig, use_container_width=True)
else:
    # ==============================
# === PARTE 3: Resultado final, odds, stake e exportação ===
# ==============================
